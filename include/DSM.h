...
public:
  ...
  static TestDSM *getInstance(const DSMConfig &conf);
  ...
  uint64_t getDSMSize() { return conf.dsmSize; }
  uint64_t getDSMBaseAddr() { return baseAddr; }
...
private:
  ...
  static thread_local TestBuffer rbuf[MAX_CORO_NUM];
...
public:
  ...
  TestBuffer &get_rbuf(int coro_id) { return rbuf[coro_id]; }
...
inline GlobalAddress TestDSM::alloc(size_t size, bool align) {
  thread_local int cur_target_node = (this->getMyThreadID() + this->getMyNodeID()) % MEMORY_NODE_NUM;
  thread_local int cur_target_dir_id = (this->getMyThreadID() + this->getMyNodeID()) % NR_DIRECTORY;
  if (++cur_target_dir_id == NR_DIRECTORY) {
    cur_target_node = (cur_target_node + 1) % MEMORY_NODE_NUM;
...
inline void TestDSM::alloc_nodes(int node_num, GlobalAddress *addrs, bool align) {
  for (int i = 0; i < node_num; ++ i) {
    addrs[i] = alloc(define::allocationPageSize, align);
  }
}

inline void TestDSM::free(const GlobalAddress& addr, int size) {
  local_allocators[addr.nodeID][0].free(addr, size);
}
